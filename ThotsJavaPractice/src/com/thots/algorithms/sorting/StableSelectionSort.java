...
/**
 * Ex : Input : 4A 5 3 2 4B 1 Output : 1 2 3 4B 4A 5
 * 
 ...
 * which are equal keys. Which makes them out of desired order. Selection sort
 * can be made Stable if instead of swapping, the minimum element is placed in
 * its position without swapping i.e. by placing the number in its position by
 * pushing every element one step forward.
 * 
...
public class StableSelectionSort {
	...
	/**
	 * Algo : set i at start of the array, start off from i+1 using j Find minimum
	 * index from j till end and mark it for swapping with element at i O(n^2) -
	 * worst case
	 * 
	 * @param inputArray
	 */
	private static void performStableSelectionSort(int[] inputArray) {
		...
		for (int i = 0; i < arrayLength - 1; i++) {
			...
			if (minIndex != i) {
				// remember the element at minIndex and shift elements from i till minIndex to
				// the right
